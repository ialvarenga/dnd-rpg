...
static func _test_default_library_has_ordered_knight_content(failures: Array[String]) -> void:
	...
	_expect(defs.ordered_actor_ids() == [&"knight", &"raider", &"archer"], "ordered_actor_ids did not match the fixed actor manifest order", failures)
	_expect(defs.ordered_item_ids() == [&"longsword", &"leather_armor", &"dagger", &"healing_potion", &"shortbow"], "ordered_item_ids did not match the fixed item manifest order", failures)
...
static func _test_equipment_aggregates_over_base_stats(failures: Array[String]) -> void:
	...
	var leather_armor := defs.get_item(&"leather_armor")
	...
	_expect(Equipment.aggregate_armor_class(actor, defs) == knight.armor_class + leather_armor.armor_class_bonus, "equipped armor_class did not add the armor's bonus", failures)

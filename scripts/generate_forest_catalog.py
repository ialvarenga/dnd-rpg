...
PALETTES = {"tree": (1, 2, 3, 4, 5), "bush": (1, 2, 3, 4, 5), "grass": (1, 2, 3, 4), "rock": (1, 2, 5)}
LEGACY = {
...
palette_index = %s
''' % (model.id, model.scene_path, tags, model.radius, model.slope, str(model.blocks).lower(), model.offset_y, model.palette_index)
...
def main() -> None:
    ...
    discovered = models()
    if len(discovered) != 137:
        raise SystemExit("Expected 137 catalogued models, found %d; run import_forest_pack.py first" % len(discovered))
